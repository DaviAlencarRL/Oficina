...
db_config = {
    ...
    'database': os.getenv('DB_NAME', 'oficina'),
...
def get_db_connection():
    try:
        ...
        database='oficina' # Optional: omit if just connecting to the server
    )
        return connection
    except Error as e:
...
def excluir_veiculo(id):
    ...
    if conn:
        ...
        try:
            # Desativa o cliente (soft delete)
            cursor.execute("UPDATE veiculos SET ativo = 0 WHERE id = %s", (id,))
            conn.commit()
            flash('Veiculo removido com sucesso!', 'success')
        except Error as e:
            flash(f'Erro ao excluir veiculo: {e}', 'danger')
        finally:
            cursor.close()
            conn.close()
    return redirect(url_for('veiculos'))
...
def salvar_os():
    ...
    try:
        if os_id:  # Atualização
            ...
            print(cliente_id, veiculo_id, usuario_id, mecanico_id, status, problema_relatado, diagnostico, observacoes)
            sql = """
